...
handle_http_methods(function () {
    POST(["postId", "time"], function ($postId, $time) {
        global $dbHandle;
        session_start();
        $user = $_SESSION["user"];
        if (!$user) {
        ...
        $result = pg_query_params($dbHandle, "INSERT INTO likes (time, author, post) VALUES ($1, $2, $3) ON CONFLICT (author, post) DO NOTHING;", array($time, $author, $postId));
        if (!$result) respond_server_error(500, "An error occurred liking the post");
        respond_with_success(array("success" => "Liked post"));
    });
});
